...
let drop ~observer ~actor ~patient =
	...
	in
		Session.emitl observer (render ~actor ~patient ~person s)
...
let level_change ~observer ~actor ~level ~rise =
	...
	let fmt = "%Au %vperf:" ^ v ^ " to level " ^ lev ^ "." in
		if observer == actor
		then Session.emitl actor (render ~actor fmt)
		else Session.emitl actor (render ~actor fmt)

let combat_result ~observer ~result =
	let s = Combat_state.string_of_response result in
		Session.emitl observer ("Damage report: " ^ s)

let logon ~observer ~actor =
	if observer != actor
	then Session.emitl observer (render ~actor "%Av %vperf:log on.")	

let logout ~observer ~actor =
	if observer != actor
	then Session.emitl observer (render ~actor "%Av %vperf:log out.")

let do_death ~observer ~actor ~cause =
	if observer != actor
	then Session.emitl observer (render ~actor "%Av %vperf:be killed.")
...
let new_sun ~observer sun_state =
	...
	let sun_msg = sun_msg ^ "." in (* should also check terrain, for "outside"*)
		Session.emitl observer sun_msg
...
let dispatch_events () =
	...
	let f ev =
		List.iter (fun recipient -> 
			dispatch_mudobject_event ~recipient ev) (Session.current_players ())
	in
		Queue.iter f events
